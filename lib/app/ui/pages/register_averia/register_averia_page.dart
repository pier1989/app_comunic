...
class RegisterAveria extends StatefulWidget {
  @override
  _CustomScrollViewComponentState createState() =>
      _CustomScrollViewComponentState();
}

class _CustomScrollViewComponentState extends State<RegisterAveria> {
  File? file;
  ...
  late String imgurls;
  updaDataimg() async {
    final postImageRef = FirebaseStorage.instance.ref().child("post");
  ...
  Future<void> saveToDataBase(String url) async {
    ...
    await FirebaseFirestore.instance.collection("averias").add(data);
  ...
  senData() async {
    if (formkey.currentState!.validate()) {
      ...
      await FirebaseFirestore.instance.collection("averias").add(
          {'name': name.text, 'img': imgurl.toString(), 'detail': detail.text});
    }
  }
